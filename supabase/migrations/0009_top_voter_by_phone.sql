-- =====================================================================
-- Migration 0009 — Rank voters/supporters by WhatsApp number (voter_phone)
-- instead of email. Name shown is taken from the latest vote record.
-- =====================================================================
...
as $$
  ...
  q as (
    select s.voter_phone,
           count(*)                       as quests,
           coalesce(sum(qu.point), 0)     as quest_points
...
-- Supporters per participant — also keyed by phone now.
...
as $$
  ...
  quest_pts as (
    select s.voter_phone, coalesce(sum(q.point), 0) as pts
    from public.submissions s
  ...
  combined as (
    select coalesce(v.voter_phone, qp.voter_phone) as phone,
           coalesce(v.nm, qp.voter_phone)          as nm,
           coalesce(v.votes, 0)                    as votes,
